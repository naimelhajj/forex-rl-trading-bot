...
class EnvironmentConfig:
    ...
    spread: float = 0.00015  # 1.5 pips (tighter, EURUSD-like)
    commission: float = 7.0  # $7 per lot per side (realistic)
...
class AgentConfig:
    ...
    eval_epsilon: float = 0.03   # PHASE-2.8: Reduce from 0.05 to 0.03 (less eval probing)
    eval_tie_only: bool = True   # QUALITY: Only apply eval_epsilon on Q-value ties
    eval_tie_tau: float = 0.035  # PHASE-2.8d Fix Pack D2.A: Tighten from 0.05 to 0.035 (reduce tie band stickiness)
...
class Config:
    ...
    VAL_STRIDE_FRAC: float = 0.15       # GATING-FIX: ~90-bar stride (15% of 600)
